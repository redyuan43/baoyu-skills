...
对 Codex 的视觉工作，一个很实用的默认配置是：在视觉类 skill 的 `EXTEND.md` 里设 `preferred_image_backend: ask`。这样可以保留 skill 自己的内容准备流程，但在最终位图出图时固定询问一次后端，在内建 `$imagegen` 和 `baoyu-danger-gemini-web` 之间选择。
...
**Codex 说明**：在 Codex CLI 里，不再把这个 skill 作为主视觉路径。Codex 的视觉工作更推荐先做内容准备和 prompt 准备，然后位图图片在内建 `$imagegen` 与 `baoyu-danger-gemini-web` 之间询问一次后端；如果最终产物是网页 / HTML，则直接走 Gemini Web 生成 HTML。`baoyu-imagine` 保留给非 Codex runtime、API/provider 定向调用和批量任务。
...
**Codex 视觉路径**：
- 位图图片：保留 skill 里的分析与 prompt 准备，最终在内建 `$imagegen` 和 `baoyu-danger-gemini-web` 之间询问一次后端
- 网页 / HTML：由 Gemini Web 直接生成 HTML，再由 Codex 做最小清洗与落盘
- `baoyu-imagine` 继续可用，但不再是 Codex 主路径里的推荐终点
