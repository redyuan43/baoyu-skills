...
Use this skill's outline, audience/style selection, and prompt-writing flow as the shared preparation layer. Then choose the final terminal by user intent:

- If the user clearly wants bitmap slide images / deck export → stay on the image terminal and resolve the backend via `## Image Generation Tools`
- If the user clearly wants a webpage / HTML version of the deck → switch to the web terminal: keep the same prepared outline and prompts, ask Gemini Web to generate one scrollable HTML page with one section per slide, then let Codex do minimal cleanup and save
