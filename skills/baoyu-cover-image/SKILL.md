...
Use this skill's analysis, dimension selection, and prompt-writing flow as the shared preparation layer. Then choose the final terminal by user intent:

- If the user clearly wants a bitmap image (cover image, poster, social image, hero image) → stay on the image terminal and resolve the backend via `## Image Generation Tools`
- If the user clearly wants a webpage / landing page / HTML adaptation → switch to the web terminal: keep the same prepared prompt and references, ask Gemini Web to draft the HTML, then let Codex do minimal cleanup and save
